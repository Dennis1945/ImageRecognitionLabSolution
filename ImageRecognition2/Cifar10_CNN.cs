...
namespace ImageRecognition2
{
    ...
    {
        static readonly string modelPath = Path.Combine(System.IO.Path.GetFullPath(@"..\..\..\"), "Model");
        static readonly string h5Path = Path.Combine(modelPath, "model.h5");
        static readonly string weightsPath = Path.Combine(modelPath, "weights.h5");
        ...
        public static string Predict(string imagePath)
        {
            ...
            var model = Sequential.LoadModel(h5Path);
